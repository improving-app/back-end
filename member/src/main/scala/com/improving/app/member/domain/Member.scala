...
object Member extends StrictLogging {
  ...
  private val tracer = Tracer("Member")
  ...
  private def registerMember(
    ...
    val span = tracer.startSpan(s"registerMember")
    try {
  ...
  private def activateMember(
    ...
    val span = tracer.startSpan(s"activateMember")
    try {
  ...
  private def suspendMember(
    ...
    val span = tracer.startSpan(s"suspendMember")
    try {
      val newMeta = meta.copy(
  ...
  private def terminateMember(
    ...
    val span = tracer.startSpan(s"terminateMember")
    try {
      val newMeta = meta.copy(
  ...
  private def editMemberInfo(
    ...
    val span = tracer.startSpan(s"editMemberInfo")
    try {
      val newMeta = meta.copy(
  ...
  private def getMemberInfo(
    ...
    val span = tracer.startSpan(s"getMemberInfo")
    try {
      info match {
        case Left(editable) =>
